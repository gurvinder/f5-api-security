...
2. Click **Add HTTP Load Balancer**
   ...
   - **Domain Name:** `your-external-llamastack.com`
...
```bash
curl -sS http://your-external-llamastack.com/v1/openai/v1/models | jq
```
Expected Output:
```json
{
...
1. **Navigate to the Swagger UI**  
   ...
   `http://your-external-llamastack.com/docs#/default/chat_completion_v1_inference_chat_completion_post`
...
```bash
curl -X 'POST' 'http://your-external-llamastack.com/v1/inference/chat-completion' \
...
1. Use a browser or `curl` to test the Shadow API endpoint:  
   - **Shadow API:** `GET http://your-external-llamastack.com/v1/version`  
...
2. *(Optional)* Use the Quickstart’s Swagger UI to confirm the endpoint is exposed:  
   - **Swagger UI:** `http://your-external-llamastack.com/docs`
...
1. Test using browser or `curl`:  
   - `GET http://your-external-llamastack.com/v1/version`
...
### Scenario
An LLM inference endpoint, running on OpenShift AI and exposed at [http://your-external-llamastack.com](http://your-external-llamastack.com), experiences performance degradation due to a high volume of requests from a single client source, potentially caused by accidental loops or intentional abuse.  
...
1. **Navigate to the Swagger URL:**  
   Open a browser tab and navigate to [http://your-external-llamastack.com/docs](http://your-external-llamastack.com/docs).
...
2. **Manage and Edit Configuration:**  
   Locate the HTTP Load Balancer serving `your-external-llamastack.com`. Click the three dots (…) in the *Action* column, then select **Manage Configuration → Edit Configuration**.
...
1. **Return to Swagger UI:**  
   Navigate to the API documentation URL (or refresh the page): [http://your-external-llamastack.com/docs](http://your-external-llamastack.com/docs).
2. **Access the Target Endpoint:**  
